...
import production.logcat.myapplication.LocationSelection
import production.logcat.myapplication.R
import production.logcat.myapplication.adapter.StandAdapter
import production.logcat.myapplication.models.*
    ...
    private fun setData(){
        ...
        if(allStandDataJSON!!.compareTo("no data")==0){
            ...
            for(key in allDocMap!!.keys){
                ...
                standList.add(StandModel(standObject.standName,if(standObject.testMode){"Test Mode"}else{"Production Mode"},(distance.toInt()).toString(),standObject.landMark,nextPage = { standNameArg, standLandMarkArg->
                    val action= BookingWindowDirections.actionBookingWindowToDriverWindow()
